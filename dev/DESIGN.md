...
## Hero film playback
The hero is the restaurant's own Vimeo film in one background iframe, driven over Vimeo's postMessage API. One piece of
state decides everything (`initFilm`): the film plays only when the visitor hasn't paused it, the hero is on screen and
the tab is visible. Every lifecycle event — player ready, a pause from the player itself, scrolling away or back, tab
hide/show, back/forward-cache restore — ends in the same `reconcile()`; nothing else sends play or pause.

- **The poster is the truth until the film moves.** `has-video` is added on two consecutive, advancing time reports, not
  on the player's "ready". A player that loads but never starts can no longer look like a frozen frame.
- **Bounded recovery.** If a play request doesn't produce a moving clock, the film is asked twice more (4s, then 3s);
  after that the poster stays, the control offers "Play film", and the retries stop. If the player starts late anyway,
  playback takes over and the poster fades out.
- **Visitor intent wins.** A visitor's pause is never undone by visibility, scrolling or recovery; only the control
  resumes it.
- **Chapters seek once.** Entering a chapter seeks after a short settle; scrubbing through the whole story is a handful
  of seeks, and playback continues from there.
- **When it goes in.** The iframe is inserted on `load` (no extra delay), with `preconnect` to player.vimeo.com covering
  the handshake. It cannot go in earlier: the film sits inside the pinned element, and ScrollTrigger's first refresh
  re-parents that subtree, which reloads the iframe and throws.
...
- **Style**: `dev/public/map/casita-light.json`, generated by `scripts/map-style.mjs` from OpenFreeMap's Positron and
  re-coloured to warm bone paper, sage streets, pale Kelly parks and deep agave labels (boundaries, shields, aeroways and
  country labels dropped). Tiles, glyphs and sprites stay on OpenFreeMap: no key, commercial use allowed, attribution
  shown under the map.
- **Behaviour**: MapLibre loads as its own chunk only when the section comes within 800px (a ScrollTrigger, so the pinned
  sections above can resize without the map missing its cue). Flat and north-up — no tilt, rotation, fly-in or satellite.
  Page scroll stays page scroll: zoom needs ⌘/Ctrl, panning on touch needs two fingers.
- **Fallback**: a static image of the same map sits under the canvas and stays if WebGL, the chunk or the tiles are
  unavailable, and on Save-Data the live map never loads at all.
- **Marker**: a Kelly dot with a soft halo; its label (name and address) opens on hover, focus or tap.
- **Hours**: unchanged schedule and logic; today's row now carries a thin Kelly rule, a faint Kelly wash and brighter
  type, and the open/closed line has a small dot — Kelly when open, brass when closed.
